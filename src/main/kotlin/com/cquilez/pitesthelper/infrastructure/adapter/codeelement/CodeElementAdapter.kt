package com.cquilez.pitesthelper.infrastructure.adapter.codeelement
...
class CodeElementAdapter(project: Project) : CodeElementPort {
    ...
    override fun findCodeElements(nodes: List<Path>): Pair<List<CodeElement>, List<String>> {
        ...
        nodes.forEach { nodePath ->
            ...
            if (nodePath.isDirectory()) {
                val containedSourceFolder = findSourceFolderContainedIn(nodePath)
                if (containedSourceFolder != null) {
                    handleFolderWithSourceFolders(nodePath, containedSourceFolder, errorMessages, result)
                    return@forEach
                }
            }
    ...
    private fun handleSourceFolderPath(
        ...
        findBasePackage(sourceFolder, errorMessages, nodePath, result)
    }

    private fun findBasePackage(
    ...
    private fun handleFolderWithSourceFolders(
        nodePath: Path,
        sourceFolder: SourceFolder,
        errorMessages: MutableList<String>,
        result: MutableList<CodeElement>
    ...
    private fun findSourceFolderContainedIn(path: Path): SourceFolder? {
        return cacheService.sourceFolderCache.values.firstOrNull { sourceFolder ->
